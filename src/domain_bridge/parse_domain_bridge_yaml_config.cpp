  ...
  DomainBridgeConfig & domain_bridge_config)
{
  ...
  if (config["topics"]) {
    ...
    for (const auto & action_node : config["actions"]) {
      // Parse keys for a action bridge
      const std::string action = action_node.first.as<std::string>();

      auto action_info = action_node.second;
      if (action_info.Type() != YAML::NodeType::Map) {
        throw YamlParsingError(file_path, "expected map value for each action");
      }

      if (!action_info["type"]) {
        throw YamlParsingError(file_path, "missing 'type' for action '" + action + "'");
      }
      const std::string type = action_info["type"].as<std::string>();

      std::size_t from_domain_id = default_from_domain;
      if (action_info["from_domain"]) {
        from_domain_id = action_info["from_domain"].as<std::size_t>();
      } else {
        if (!is_default_from_domain) {
          throw YamlParsingError(file_path, "missing 'from_domain' for action '" + action + "'");
        }
      }

      std::size_t to_domain_id = default_to_domain;
      if (action_info["to_domain"]) {
        to_domain_id = action_info["to_domain"].as<std::size_t>();
      } else {
        if (!is_default_to_domain) {
          throw YamlParsingError(file_path, "missing 'to_domain' for action '" + action + "'");
        }
      }

      // Parse action bridge options
      ActionBridgeOptions options;
      if (action_info["remap"]) {
        options.remap_name(action_info["remap"].as<std::string>());
      }
      domain_bridge_config.actions.push_back({{action, type, from_domain_id, to_domain_id}, options});
    }
  }
}
